...
- 按文末“Git 与提交”的发布约定完成验证、提交和部署；问题修复发布到生产，新需求发布到使用示例数据的独立服务。
...
- AI 通过服务端的 PiAgent 与 Pi AI 请求用户配置的 OpenAI Compatible `/chat/completions` 端点；浏览器只创建、查询和取消服务端 AI 任务，不得重新直连模型供应商。
...
- 多步工具调用必须在 PiAgent 模型请求层设置明确上限，接近上限时禁用工具并要求收束为最终回答，防止模型陷入无休止调用；工具必须使用 schema 校验参数。
...
- 预览服务为临时验收环境；用户确认验收完成后可删除。保留明确的停止与清理方式，清理只针对该预览服务及其示例数据，不影响生产。
- 不执行强制推送或历史重写；超出上述发布流程的远程操作仍需用户明确授权。
