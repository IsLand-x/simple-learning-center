...
- 手机和平板竖屏使用独立响应式视图：书架和设置采用底部导航，书架以双列封面展示；阅读页底部直接提供目录、上一页、下一页和更多功能入口，支持左右滑动翻页和原生文字选区，长按选字与拖动选区时不会误触翻页；目录从侧边滑入，更多功能中的 AI、笔记、高亮、评论和阅读样式共用约九成屏高的动画底部抽屉，不压缩正文宽度；浮层打开时，左右滑动、浏览器或系统返回键会优先收起浮层。
