...
import { INFOGRAPHIC_PRESETS } from './knowledgeMaps/presets.mjs';

const plan = {
  title: '复制方案对比', purpose: '解释当前章节中的两种复制方式',
  preset: 'comparison', selection_reason: '读者问两种复制方式的区别，统一维度便于比较。',
  layout: '横向对比图，左侧同步，右侧异步',
  nodes: [
    { id: 'sync', label: '同步复制', detail: '等待副本确认', kind: 'paraphrase', source_ids: ['s1'] },
    { id: 'async', label: '异步复制', detail: '不等待副本确认', kind: 'paraphrase', source_ids: ['s1'] },
  ],
  relationships: [{ from: 'sync', to: 'async', label: '比较延迟与一致性' }],
  sources: [{ id: 's1', location: '当前章节：复制', excerpt: '同步复制等待副本确认；异步复制不等待副本确认。' }], limitations: '仅覆盖本次读取的章节片段',
...
test('规划校验节点和关系；生图必须使用本次有效方案', async () => {
  ...
  const tools = createInfographicTools({
    generateImage: async () => { generated++; return Buffer.from('fixture'); },
    save: async () => imageUrl,
  });
  ...
  for (const cancel of [false, true]) {
    ...
    const tools = createInfographicTools({
      ...
      generateImage: async ({ prompt, signal }) => {
        ...
        calls++;
        if (cancel) { controller.abort(); return Buffer.from('fixture'); }
...
test('六种预设分别使用对应绘图规则，并保留信息类型和逐项来源', async () => {
  assert.deepEqual(Object.keys(INFOGRAPHIC_PRESETS), ['concept', 'mechanism', 'comparison', 'argument', 'timeline', 'application']);
  for (const [preset, definition] of Object.entries(INFOGRAPHIC_PRESETS)) {
    let draft = '';
    const tools = createInfographicTools({
      onPlan: (value) => { draft = value; },
    ...
    const { plan_id, outline } = await tools.plan_infographic.execute({ ...plan, preset, follow_up_topics: ['副本故障处理'] });
...
test('不能伪装原文、遗漏来源、使用未知预设或用过量文字塞满图片', async () => {
  ...
  const invalidPlans = [
    ...
    { ...plan, nodes: [{ ...plan.nodes[0], kind: 'quote', detail: '没有任何代价' }, plan.nodes[1]] },
    { ...plan, nodes: [{ ...plan.nodes[0], source_ids: ['missing'] }, plan.nodes[1]] },
    { ...plan, sources: [plan.sources[0], plan.sources[0]] },
    { ...plan, nodes: Array.from({ length: 8 }, (_, index) => ({ ...plan.nodes[index % 2], id: index < 2 ? plan.nodes[index].id : `n${index}`, detail: '信息'.repeat(100) })) },
  ];
  for (const invalid of invalidPlans) await assert.rejects(tools.plan_infographic.execute(invalid));
  const valid = await tools.plan_infographic.execute({ ...plan, nodes: [{ ...plan.nodes[0], kind: 'quote' }, plan.nodes[1]] });
...
const options = {
  ...
  conversationId: 'infographic-test', messages: [{ role: 'user', content: '请生成复制方式的信息图' }],
  currentText: '同步复制等待副本确认；异步复制不等待。', notes: [], highlights: [], readingSessions: [],
};
...
test('真实 PiAgent 工具循环先读正文再规划与生图，方案和图片进入最终及进度消息', async () => {
  ...
  faux.setResponses([
    (context) => {
      ...
      return fauxAssistantMessage(fauxToolCall('read_current_chapter', {}), { stopReason: 'toolUse' });
    },
  ...
  const result = await runServerAiChat({
    ...options, oauth: { runtime: async () => runtime },
  ...
  assert.ok(progress.some((value) => value.dialogueContent.some((item) => item.name === '信息图' && item.status === 'completed')));
});

test('非 ChatGPT 供应商或非阅读场景不提供信息图工具', async () => {
  for (const [provider, resourceType] of [['kimi-coding', 'book'], [undefined, 'book'], ['openai-codex', 'video']]) {
    const faux = fauxProvider({ tokensPerSecond: 0 });
    ...
    const runtime = runtimeFor(faux);
    await runServerAiChat({ ...options, resourceType, config: { oauthProvider: provider },
      oauth: { runtime: async () => runtime }, runtimeFactory: () => runtime,
    });
  }
});
...
test('生图失败保留方案和真实失败状态，并阻止改用全景工具再次收费', async () => {
  ...
  faux.setResponses([
    ...
    (context) => {
      const result = context.messages.filter((message) => message.role === 'toolResult').at(-1);
      const { plan_id } = JSON.parse(result.content[0].text);
      return fauxAssistantMessage(fauxToolCall('generate_infographic', { plan_id }), { stopReason: 'toolUse' });
    },
    fauxAssistantMessage(fauxToolCall('generate_book_knowledge_map', {}), { stopReason: 'toolUse' }),
    fauxAssistantMessage('信息图生成失败，请检查额度后重新发送请求。'),
  ]);
  const result = await runServerAiChat({
    ...options, oauth: { runtime: async () => runtimeFor(faux) },
    infographicImageGenerator: async () => { throw new Error('ChatGPT 订阅生图未成功'); },
    knowledgeMapGenerator: () => assert.fail('不能切换工具重复生图'),
  });
  assert.match(result.content, /信息图内容稿/);
  assert.match(result.content, /信息图生成失败/);
  assert.ok(!result.content.includes('![信息图]'));
  assert.ok(result.dialogueContent.some((item) => item.name === 'generate_infographic' && item.status === 'failed'));
});
