...
import { saveKnowledgeMap } from './service.mjs';
import { INFOGRAPHIC_PRESETS, INFOGRAPHIC_GUIDANCE } from './presets.mjs';
...
function validate(schema, params) {
  try {
    return validateToolArguments(
      { parameters: schema },
      { name: 'infographic', arguments: params },
    );
  } catch {
...
const kindLabels = { quote: '原文', paraphrase: '原文概括', explanation: '补充解释' };
function outlineFor(plan) {
  const labels = new Map(plan.nodes.map((node) => [node.id, node.label]));
  return [
    `**信息图内容稿：${plain(plan.title)}**`,
    `目标：${plain(plan.purpose)}`,
    `图型：${INFOGRAPHIC_PRESETS[plan.preset].label}。${plain(plan.selection_reason)}`,
            ...
            .then(async () => {
              ...
              const png = await generateImage({
                ...
                prompt: [
                  ...
                  `预设：${INFOGRAPHIC_PRESETS[planned.plan.preset].label}。${INFOGRAPHIC_PRESETS[planned.plan.preset].drawing}`,
