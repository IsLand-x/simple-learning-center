...
const rssDigestScheduler = createRssDigestScheduler({
  startDigest: (input) => aiJobManager.startDigest(input),
});
...
const server = serve(
  ...
  () => {
    ...
    console.log(
      `RSS 服务端刷新周期：${Math.round(RSS_REFRESH_INTERVAL_MS / 60_000)} 分钟（错峰执行）`,
    );
    console.log(
      `YouTube 服务端请求：${YOUTUBE_PROXY_CONFIGURED ? '使用已配置代理' : '直接连接（不会自动继承浏览器代理）'}`,
    );
  },
);
