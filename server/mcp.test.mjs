...
test('MCP HTTP 直连可调用全部书架工具，并保持权限和数据隔离', async () => {
  ...
  const token = (await (await app.request(tokenPath, { method: 'POST', headers: { 'X-Learning-Center-Request': '1' } })).json()).token;
  ...
  assert.equal((await remote.request(`${tokenPath}/mcp`, { headers: { Authorization: `Bearer ${token}` } })).status, 401);
  ...
  try {
    ...
    assert.deepEqual(tools.tools.map((tool) => tool.name), ['list_book', 'edit_book', 'list_book_highlight_and_comment', 'read_book_note', 'trash_book', 'list_book_list', 'edit_book_list', 'upload_book']);
    ...
    assert.equal((await client.callTool({ name: 'edit_book', arguments: { book_id: 'b1', title: '不允许', book_list_ids: [] } })).isError, true);
    assert.equal((await client.callTool({ name: 'edit_book', arguments: { book_id: 'b1', book_list_ids: ['missing'] } })).isError, true);
    const oldState = await readPersistedState();
    assert.equal((await call('edit_book', { book_id: 'b1', book_list_ids: ['l1'] })).book.bookLists[0].id, 'l1');
    ...
    assert.equal((await call('edit_book_list', { book_list_id: 'l1', expected_updated_at: list.updatedAt, name: '新书单', note: '说明' })).bookList.name, '新书单');
    assert.equal((await client.callTool({ name: 'edit_book_list', arguments: { book_list_id: 'l1', expected_updated_at: list.updatedAt, name: '覆盖' } })).isError, true);
    assert.equal((await call('list_book_highlight_and_comment', { book_id: 'b1' })).items[0].comment, '见解');
