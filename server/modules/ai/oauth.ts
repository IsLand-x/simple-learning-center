...
interface OAuthSession {
  ...
  public: { userCode?: string; url?: string; expiresAt?: number; message?: string };
  done?: Promise<void>;
}
...
export function createOAuthService({
  ...
  timeoutMs = 15 * 60_000,
}: { path?: string; providers?: Provider[]; timeoutMs?: number } = {}) {
  ...
  const read = async () => {
    try {
      const data: Record<string, Credential> = JSON.parse(await readFile(path, 'utf8'));
      if (
        !data ||
        typeof data !== 'object' ||
        Array.isArray(data) ||
  ...
  return {
    ...
    async start(id: string) {
      ...
      const session: OAuthSession = { controller, state: 'pending', public: {} };
    ...
    async runtime(id: string, modelId: string, signal?: AbortSignal) {
      providerFor(id);
