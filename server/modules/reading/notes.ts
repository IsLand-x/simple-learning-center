import type { NoteItem } from '../../../contracts/domain.js';
...
function notesForBook(state: StoredState, bookId: string) {
  return (Array.isArray(state.notes) ? state.notes : [])
    .filter((note) => note?.bookId === bookId)
    .sort((left, right) => Number(left.createdAt || 0) - Number(right.createdAt || 0));
}

function publicNote(note: NoteItem) {
  return {
    id: note.id,
    title: note.title,
...
export async function readBookNotes(bookId: unknown) {
...
export async function createBookNote(bookId: unknown, bookTitle: unknown, content: unknown) {
  const normalizedBookId = requireIdentifier(bookId, '书籍');
...
export async function updateBookNote(
  ...
  return mutatePersistedState((persistedState) => {
    ...
    const index = notes.findIndex(
      (note) => note?.id === normalizedNoteId && note?.bookId === normalizedBookId,
    );
