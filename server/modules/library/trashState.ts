import type { BookItem, TrashedBookItem } from '../../../contracts/domain.js';
...
function stateArray<T, K extends keyof T>(
  state: T | undefined,
  key: K,
): Extract<NonNullable<T[K]>, unknown[]> {
  return (Array.isArray(state?.[key]) ? state[key] : []) as Extract<NonNullable<T[K]>, unknown[]>;
}

function entryBookId(entry: TrashedBookItem | undefined) {
  return typeof entry?.book?.id === 'string' ? entry.book.id : '';
}

function bookUpdatedAt(book: BookItem | undefined) {
  return Number.isFinite(book?.updatedAt) ? book!.updatedAt : 0;
}

function bumpStateVersion(persistedState: PersistedState) {
...
export function protectBookTrashStateFromClient(
  ...
  const mergedBooks = new Map<string, BookItem>();

  for (const book of currentBooks) {
    if (typeof book?.id === 'string' && !unavailableBookIds.has(book.id))
  ...
  const incomingOrder = incomingBooks
    .map((book) => book?.id)
    .filter((bookId) => mergedBooks.has(bookId));
  ...
  const permanentlyDeletedBookIds = new Set(
    currentTombstones.map((entry) => entry?.bookId).filter(Boolean),
  );
  for (const key of [
    'highlights',
    ...
    const currentTrashedItems = stateArray(currentPersistedState.state, key).filter((item) =>
      trashedBookIds.has(item?.bookId),
    );
    (protectedState.state as Record<string, unknown>)[key] = [
      ...structuredClone(currentTrashedItems),
      ...stateArray(protectedState.state, key).filter(
        (item) => !trashedBookIds.has(item?.bookId) && !permanentlyDeletedBookIds.has(item?.bookId),
      ),
    ];
  }
  ...
  const currentLists = new Map(
    stateArray(currentPersistedState.state, 'bookLists').map((list) => [list.id, list]),
  );
  protectedState.state.bookLists = stateArray(protectedState.state, 'bookLists').map((bookList) => {
    ...
    const latest =
      currentList && currentList.updatedAt > bookList.updatedAt ? currentList : bookList;
    return {
      ...latest,
      bookIds: stateArray(latest, 'bookIds').filter((bookId) => activeBookIds.has(bookId)),
    };
  });
...
export function moveBookToTrashInState(
  persistedState: PersistedState,
  bookId: string,
  deletedAt = Date.now(),
) {
  ...
  state.deletedBookTombstones = stateArray(state, 'deletedBookTombstones').filter(
...
export function restoreBookFromTrashInState(
  persistedState: PersistedState,
  bookId: string,
  restoredAt = Date.now(),
) {
  const state = persistedState?.state;
  if (!state) return null;
  const trashedBook = stateArray(state, 'trashedBooks').find(
    (entry) => entryBookId(entry) === bookId,
  );
  if (!trashedBook) return null;

  const activeBooks = stateArray(state, 'books');
  if (!activeBooks.some((book) => book?.id === bookId))
    state.books = [trashedBook.book, ...activeBooks];
  state.trashedBooks = stateArray(state, 'trashedBooks').filter(
    (entry) => entryBookId(entry) !== bookId,
  );
  const positions = new Map(
    stateArray(trashedBook, 'bookListPositions').map((position) => [
      position.bookListId,
      position.index,
    ]),
  );
  state.bookLists = stateArray(state, 'bookLists').map((bookList) => {
    ...
    if (!Number.isInteger(savedIndex) || stateArray(bookList, 'bookIds').includes(bookId))
      return bookList;
...
export function permanentlyDeleteBookInState(
  persistedState: PersistedState,
  bookId: string,
  deletedAt = Date.now(),
) {
  ...
  const wasPresent =
    stateArray(state, 'books').some((book) => book?.id === bookId) ||
  ...
  for (const key of [
    ...
    (state as Record<string, unknown>)[key] = stateArray(state, key).filter(
...
export function expiredTrashedBookIds(persistedState: PersistedState | null, now = Date.now()) {
  return stateArray(persistedState?.state, 'trashedBooks')
    .filter(
      (entry) =>
        Number.isFinite(entry?.deletedAt) && now - entry.deletedAt >= BOOK_TRASH_RETENTION_MS,
    )
    .map(entryBookId)
    .filter(Boolean);
}
