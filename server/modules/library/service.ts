...
export function requireBook(state: StoredState, id: string) {
  const book = (state.books || []).find((item) => item.id === id);
  if (!book) throw statusError(404, '书籍不存在或已在回收站中');
  return book;
}

export async function libraryState(hydrateNote: (note: NoteItem) => boolean = () => false) {
  const persisted = await readPersistedState({ hydrateNote });
  if (!persisted?.state) throw statusError(409, '请先打开应用完成初始化');
  return persisted.state;
}

export function publicBook(state: StoredState, book: BookItem) {
  return {
    id: book.id,
    title: book.title,
    author: book.author,
...
export function editBook({ book_id, book_list_ids }: { book_id: string; book_list_ids: string[] }) {
