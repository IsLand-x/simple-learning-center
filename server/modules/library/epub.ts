...
// xmldom 0.8 returns empty strings for missing attributes and element text.
...
const elements = (node: Document | XmlElement, name: string) =>
  Array.from(node.getElementsByTagNameNS('*', name)) as XmlElement[];

export async function readEpubMetadata(path: string) {
  ...
  try {
    ...
    async function read(name: string, limit = XML_LIMIT) {
    ...
    const root = elements(container, 'rootfile')[0]?.getAttribute('full-path') as string;
    ...
    let toc: TocItem[] = [];
    const children = (node: XmlElement | undefined, name: string): XmlElement[] =>
      Array.from(node?.childNodes || []).filter(
        (child): child is XmlElement =>
          child.nodeType === 1 && (child as Element).localName === name,
      );
    const hrefFrom = (file: string, href: string) => {
    ...
    if (nav) {
      ...
      const mapList = (list: XmlElement | undefined): TocItem[] =>
    ...
    if (!toc.length) {
      ...
      if (ncx) {
        ...
        const mapPoints = (parent: XmlElement | undefined): TocItem[] =>
