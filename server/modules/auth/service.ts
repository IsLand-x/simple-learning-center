...
async function hashPassword(password: string, salt = randomBytes(16).toString('base64url')) {
  ...
  return { salt, hash: Buffer.from(hash as Buffer).toString('base64url') };
}

function validateStoredCredentials(value: Credentials) {
  if (
    !value ||
...
async function createCredentials(username: string, password: string, previous?: Credentials) {
  return {
...
function createSessionToken(credentials: Credentials) {
...
function verifySessionToken(token: unknown, credentials: Credentials) {
  ...
  try {
    ...
    return (
      session.revision === credentials.revision &&
  ...
  return {
    async getConfiguredUsername() {
      try {
        ...
        if (errorHasCode(error, 'ENOENT')) return defaultUsername;
        throw error;
      }
    },
    async getUsername() {
    ...
    async verifySession(token: unknown) {
