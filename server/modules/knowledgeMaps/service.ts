...
import type { BookPassage } from '../reading/search.js';
import type { PiRuntime } from '../ai/runtime.js';
import { randomUUID } from 'node:crypto';
import { join } from 'node:path';
import { loadBookPassages } from '../reading/search.js';
import { atomicWrite, knowledgeMapDirectoryPath } from '../../infrastructure/fs/files.js';
...
export function passageBatches(passages: BookPassage[]) {
  const batches = [];
  let batch = '';
...
export async function generateBookKnowledgeMap({
  ...
  load?: typeof loadBookPassages;
  summarize?: typeof analyze;
