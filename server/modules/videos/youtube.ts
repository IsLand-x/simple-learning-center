...
>;
type CaptionTrack = NonNullable<
  NonNullable<YouTubeVideoInfo['captions']>['caption_tracks']
>[number];
interface TimedTextPayload {
  events?: Array<{ segs?: Array<{ utf8?: unknown }>; tStartMs?: unknown; dDurationMs?: unknown }>;
}
...
const METADATA_CLIENTS = ['MWEB', 'WEB', 'ANDROID'] as const;
...
function playabilityError(info: YouTubeVideoInfo | undefined) {
  ...
  if (status || reason) {
    return statusError(422, `YouTube 当前不允许服务器读取该视频${reason ? `：${reason}` : ''}`, {
      expose: true,
    });
  }
  return statusError(502, 'YouTube 返回的视频信息不完整，请稍后重试', { expose: true });
}

export function parseYouTubeVideoId(input: unknown) {
...
export function parseTimedTextJson(
  payload: TimedTextPayload | null | undefined,
): VideoCaptionCue[] {
...
async function readLimitedText(response: Response, limit = MAX_CAPTION_BYTES) {
...
function captionRequestUrl(baseUrl: string, translatedLanguage?: string) {
...
function chooseCaptionTrack(tracks: CaptionTrack[]) {
  return (
    tracks.find((track) => /^en(?:-|$)/i.test(track.language_code) && track.kind !== 'asr') ??
...
export async function fetchYouTubeVideo(
  ...
  }: { fetchImpl?: typeof fetch; getClient?: typeof getYouTubeClient } = {},
) {
  const videoId = parseYouTubeVideoId(input);
  let info: YouTubeVideoInfo | undefined;
  try {
    const youtube = await getClient();
    let lastIncompleteInfo: YouTubeVideoInfo | undefined;
    let playableInfoWithoutCaptions: YouTubeVideoInfo | undefined;
    for (const client of METADATA_CLIENTS) {
      try {
        ...
        const playabilityStatus = cleanText(
    ...
    if (Number.isInteger((error as { status?: unknown } | null | undefined)?.status)) throw error;
    const transportFailure = youtubeTransportError(error);
    if (transportFailure) throw transportFailure;
  ...
  let originalCues: VideoCaptionCue[] = [];
  let chineseCues: VideoCaptionCue[] = [];
  let captionError = '';
  ...
  if (track) {
    try {
      ...
      const transportFailure = youtubeTransportError(error);
      captionError =
        transportFailure?.message || (error instanceof Error ? error.message : '字幕读取失败');
    }
  } else {
