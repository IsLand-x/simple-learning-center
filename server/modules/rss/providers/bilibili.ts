import type { FetchedRssFeed, FetchedRssItem } from '../types.js';
...
interface BilibiliItem {
  ...
  title?: unknown;
  author?: unknown;
  owner?: { name?: unknown; face?: unknown };
  name?: unknown;
  description?: unknown;
  desc?: unknown;
  rcmd_reason?: unknown;
...
interface BilibiliNavigation {
  wbi_img?: { img_url?: string; sub_url?: string };
  isLogin?: unknown;
  uname?: unknown;
}
interface BilibiliUpData {
  list?: { vlist?: BilibiliItem[] };
}
interface BilibiliSeries {
  number?: unknown;
  name?: unknown;
  subject?: unknown;
}
interface BilibiliWeekly {
  list?: BilibiliItem[];
}
...
interface CookieFetchOptions extends FetchOptions {
  cookie?: string;
}
interface WbiKeys {
  imgKey: string;
...
const wbiKeyCache = new Map<string, { value: WbiKeys; expiresAt: number }>();
...
function numericTimestamp(value: unknown, fallback: number) {
  const number = Number(value);
  if (!Number.isFinite(number) || number <= 0) return fallback;
  return number > 10_000_000_000 ? number : number * 1_000;
}

function jsonErrorForStatus(status: number) {
  if (status === 412) {
...
async function readLimitedJson<T>(response: Response): Promise<BilibiliResponse<T>> {
  ...
  try {
    return JSON.parse(new TextDecoder().decode(bytes)) as BilibiliResponse<T>;
  } catch {
    throw sourceError(502, 'UPSTREAM_UNAVAILABLE', 'B站接口返回了无法解析的数据');
  }
}

async function requestBilibiliJson<T>(
  url: string,
  {
    cookie = '',
    fetchImpl = fetch,
    referer = 'https://www.bilibili.com/',
  }: { cookie?: string; fetchImpl?: typeof fetch; referer?: string } = {},
  ...
  try {
    response = await fetchImpl(url, {
      headers: {
        ...
        ...(referer.startsWith('https://space.bilibili.com/')
...
function assertBilibiliSuccess<T>(
  payload: BilibiliResponse<T> | null | undefined,
  { authenticated = false } = {},
) {
  ...
  if (code === -101 || code === -6 || /未登录|登录失效|cookie/i.test(message)) {
    throw sourceError(
      422,
      authenticated ? 'BILIBILI_COOKIE_INVALID' : 'BILIBILI_COOKIE_REQUIRED',
      authenticated
  ...
  throw sourceError(
    ...
    `B站接口返回异常${message ? `：${message}` : `（${code}）`}`,
  );
}

function imageHtml(url: unknown, alt: string) {
  if (!url) return '';
...
function normalizedBilibiliItem(
  item: BilibiliItem | null | undefined,
  fetchedAt: number,
  extraDescription: unknown = '',
): FetchedRssItem | null {
  const bvid = compactText(
    item?.bvid || (String(item?.param ?? '').startsWith('BV') ? item?.param : ''),
    32,
  );
...
export function parseBilibiliUpInput(value: unknown) {
  ...
  if (
    url.protocol !== 'https:' ||
    url.hostname.toLowerCase() !== 'space.bilibili.com' ||
    url.username ||
    url.password
  ) {
...
function wbiKeyFromUrl(value: string | undefined) {
  try {
    return new URL(value as string).pathname.split('/').at(-1)?.split('.')[0] || '';
  } catch {
    return '';
  }
}
...
async function getWbiKeys({
  ...
  now = Date.now,
  force = false,
}: CookieFetchOptions & { force?: boolean } = {}) {
  const cacheKey = cookie ? 'authenticated' : 'anonymous';
  const cached = wbiKeyCache.get(cacheKey);
  if (!force && (cached?.expiresAt as number) > now()) return cached!.value;
  const payload = await requestBilibiliJson<BilibiliNavigation>(
    `${BILIBILI_API_ORIGIN}/x/web-interface/nav`,
    {
      cookie,
      fetchImpl,
    },
...
async function fetchBilibiliUpAttempt(
  uid: string,
  {
    cookie = '',
    fetchImpl = fetch,
    now = Date.now,
    forceKeys = false,
  }: CookieFetchOptions & { forceKeys?: boolean } = {},
) {
  ...
  const payload = await requestBilibiliJson<BilibiliUpData>(
    `${BILIBILI_API_ORIGIN}/x/space/wbi/arc/search?${query}`,
    {
      cookie,
      fetchImpl,
...
function shouldRetryWithCookie(error: unknown) {
  const sourceCode = (error as Partial<StatusError> | null | undefined)?.sourceCode;
  return (
    sourceCode === 'BILIBILI_RISK_CONTROL' ||
    sourceCode === 'BILIBILI_COOKIE_REQUIRED' ||
    sourceCode === 'UPSTREAM_UNAVAILABLE'
  );
}

export async function fetchBilibiliUp(
  uid: string,
  {
    fetchImpl = fetch,
    getCookie = async () => '',
    now = Date.now,
  }: FetchOptions & { getCookie?: () => Promise<string> } = {},
  ...
  const items = videos
    ...
    .filter((item): item is FetchedRssItem => item !== null);
  const author =
    items.find((item) => item.author)?.author ||
    compactText(videos[0]?.author, 500) ||
    `UP 主 ${uid}`;
  return {
    title: author,
...
export async function fetchBilibiliWeekly({
  fetchImpl = fetch,
  now = Date.now,
}: FetchOptions = {}): Promise<FetchedRssFeed> {
  const seriesPayload = await requestBilibiliJson<BilibiliSeries[]>(
  ...
  const listPayload = await requestBilibiliJson<BilibiliWeekly>(
...
export async function verifyBilibiliCookie(
  cookie: string,
  { fetchImpl = fetch, now = Date.now }: FetchOptions = {},
) {
  const payload = await requestBilibiliJson<BilibiliNavigation>(
    `${BILIBILI_API_ORIGIN}/x/web-interface/nav`,
    {
