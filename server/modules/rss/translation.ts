...
type TranslationItem = Pick<
  RssItem,
  'fullContentHtml' | 'contentHtml' | 'fullContentText' | 'contentText'
>;
...
function sourceHtml(item: TranslationItem | null | undefined) {
...
function hasSkippedAncestor(node: Node) {
  ...
  while (parent && parent.nodeType !== 9) {
    if (parent.nodeType === 1 && SKIPPED_TAGS.has((parent as Element).tagName)) return true;
...
function collectTextNodes(root: Node) {
  ...
  const visit = (node: Node) => {
    if (node.nodeType === 3) {
      ...
      if (text.data.trim() && !hasSkippedAncestor(text)) nodes.push(text);
      return;
    }
...
function coreRange(value: string) {
...
function nextChunkEnd(value: string, start: number) {
  ...
  return boundary >= Math.floor(MAX_SEGMENT_CHARACTERS / 2) ? start + boundary + 1 : hardEnd;
}

export function prepareRssTranslationSource(
  item: TranslationItem | null | undefined,
): TranslationSource {
  ...
  textNodes.some((node, nodeIndex) => {
    ...
    while (
      ...
      const end = Math.min(
        range.end,
        nextChunkEnd(node.data.slice(0, range.end), start),
        start + remainingBudget,
      );
      if (end <= start) break;
...
export function parseRssTranslationResponse(value: unknown, source: TranslationSource) {
  if (typeof value !== 'string' || !value.trim()) throw new Error('模型没有返回翻译结果');
  let parsed: { version?: unknown; segments?: Array<{ id?: unknown; text?: unknown }> };
  try {
...
export function renderRssTranslation(source: TranslationSource, translations: Map<string, string>) {
  const document = createDocument(source.html);
  const textNodes = collectTextNodes(document.body);
  const segmentsByNode = new Map<number, TranslationSegment[]>();
...
export function completeRssTranslation(value: unknown, source: TranslationSource) {
