...
function compactText(value: unknown, maxLength = 100_000) {
  return String(value ?? '')
    .replace(/\r/g, '')
    .replace(/[\t\f\v ]+/g, ' ')
    .replace(/\n[ \t]+/g, '\n')
...
function stripDoctype(value: string) {
...
function localName(node: (Node & { localName?: string | null }) | null | undefined) {
  return (
...
function elementChildren(node: Node | null | undefined): Element[] {
  return Array.from(node?.childNodes ?? []).filter(
    (child): child is Element => child.nodeType === 1,
  );
}

function directChild(node: Node | undefined, names: string[]) {
...
function descendants(node: Node | undefined, names: string[]) {
  const accepted = new Set(names.map((name) => name.toLocaleLowerCase()));
  const matches: Element[] = [];
  const visit = (parent: Node | undefined) => {
...
function firstDescendant(node: Node | undefined, names: string[]) {
  return descendants(node, names)[0];
}

function childText(node: Node | undefined, names: string[], maxLength?: number) {
  for (const name of names) {
    const normalized = name.toLocaleLowerCase();
    const element =
      elementChildren(node).find((child) => localName(child) === normalized) ||
...
function atomLink(node: Node, baseUrl: string) {
...
function itemLink(node: Node, baseUrl: string) {
  const link = directChild(node, ['link']);
  return (
    absoluteUrl(link?.getAttribute('href') || link?.textContent, baseUrl) || atomLink(node, baseUrl)
  );
}

function decodedHtmlUrl(value: unknown) {
  return String(value ?? '')
    .replaceAll('&amp;', '&')
    .replaceAll('&#38;', '&')
    .replaceAll('&quot;', '"')
...
function contentImages(value: unknown, baseUrl: string) {
  const images: string[] = [];
...
function itemImages(node: Node, rawContent: string, baseUrl: string) {
...
function stableItemId(value: string) {
  return createHash('sha256').update(value).digest('hex').slice(0, 32);
}

function feedItem(node: Node, baseUrl: string, fetchedAt: number): FetchedRssItem {
  ...
  const authorNode =
    directChild(node, ['author', 'creator']) || firstDescendant(node, ['author', 'creator']);
  const author =
    localName(authorNode) === 'author'
...
export function parseRssFeed(
  ...
  const itemNodes = atom
    ? elementChildren(root).filter((child) => localName(child) === 'entry')
    : descendants(channel, ['item']);
  ...
  const siteUrl = atom
    ? atomLink(channel, feedUrl)
    : absoluteUrl(childText(channel, ['link'], 4_000), feedUrl);
  const seen = new Set();
...
export async function validateRemoteUrl(value: unknown) {
  let url;
  try {
...
function decodeFeed(buffer: Buffer) {
...
export async function fetchRssFeed(
  value: unknown,
  {
    fetchImpl = fetch,
    onFetchError,
  }: { fetchImpl?: typeof fetch; onFetchError?: (error: unknown) => Error | null | undefined } = {},
