...
import { protectServerRssState } from '../state/compatibility/rssState.js';
import { fetchRssSource, resolveRssSource, sourceMinimumIntervalMs } from './sources.js';

const fetched = { title: '源', feedUrl: 'https://example.com/feed.xml', items: [] };
const channelId = 'UC1234567890123456789012';

test('统一内容源解析器返回可持久化的判别联合', async () => {
  const weekly = await resolveRssSource(
  ...
  const up = await resolveRssSource(
    { kind: 'bilibili-up', input: 'https://space.bilibili.com/946974' },
    {
  ...
  const youtube = await resolveRssSource(
    { kind: 'youtube-channel', input: '@openai' },
    {
