...
function localTimeKey(timestamp: number) {
...
export function dueDigestSchedule(
  settings: Partial<RssDigestSettings> | undefined,
  now = Date.now(),
) {
  ...
  return !Number.isFinite(settings.lastAttemptAt) ||
    now - (settings.lastAttemptAt ?? 0) >= intervalMs
...
export function createRssDigestScheduler({
  ...
  const runCycle = async () => {
    ...
    try {
      ...
      logger.warn?.(
        `RSS 日报定时任务失败：${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      running = false;
