...
interface SecretsFile {
  ...
  bilibili?: SourceCredential;
}
import { chmod, readFile, rm } from 'node:fs/promises';
import { SOURCE_SECRETS_FILE } from '../../config.js';
import { statusError } from '../../infrastructure/http/errors.js';
import { verifyBilibiliCookie } from '../rss/providers/bilibili.js';
...
function publicStatus(entry: SourceCredential | undefined) {
...
async function readSecrets(path: string): Promise<SecretsFile> {
  try {
    ...
    if (errorHasCode(error, 'ENOENT')) return { formatVersion: FORMAT_VERSION };
    throw error;
  }
}

async function writeSecrets(path: string, secrets: SecretsFile) {
...
export function createSourceSecretsService({
    ...
    queued(async () => {
      ...
      try {
        ...
        const invalid =
          ...
          'sourceCode' in error &&
          error.sourceCode === 'BILIBILI_COOKIE_INVALID';
        secrets.bilibili = {
          ...entry,
  ...
  return {
    ...
    async setBilibiliCookie(value: unknown) {
