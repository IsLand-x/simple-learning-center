...
import { searchIndexPath } from '../../infrastructure/fs/files.js';
...
export async function loadBookPassages(book: BookItem): Promise<BookPassage[]> {
  let stored: SearchIndex;
  try {
...
function queryTerms(query: unknown) {
  ...
  const bigrams =
    /[\u3400-\u9fff]/.test(compact) && compact.length > 2
...
export async function searchBookContent(book: BookItem, query: unknown, maxResults = 6) {
