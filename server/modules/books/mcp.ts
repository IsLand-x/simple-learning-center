...
import { z } from 'zod';
import { movePersistedBookToTrash } from './trash.js';
import { editBook, editBookList, libraryState, publicBook, requireBook } from './catalog.js';
...
function page<T>(items: T[], { offset, limit }: { offset: number; limit: number }) {
  return {
...
export function createLibraryMcpServer() {
  ...
  function tool<T extends z.ZodRawShape>(
    ...
    shape: T,
    readOnly: boolean,
    action: (args: z.output<z.ZodObject<T>>) => Promise<Record<string, unknown>>,
    destructive = false,
  ) {
            ...
            content: [
              {
                type: 'text',
                text:
                  details.status && details.status < 500 ? details.message : '操作失败，请稍后重试',
              },
            ],
          };
        }
