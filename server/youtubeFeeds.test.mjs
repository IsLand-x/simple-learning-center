...
} from './modules/rss/providers/youtube.js';
...
test('YouTube 频道输入支持 channel ID、handle 和频道地址', () => {
  ...
  assert.equal(
    parseYouTubeChannelInput(`https://www.youtube.com/channel/${channelId}`).channelId,
    channelId,
  );
});
    ...
    },
    {
      fetchFeed: async (url) => {
        ...
        return {
          ...
          fetchedAt: 100,
          items: [
            { id: 'legacy', title: '视频', link: 'https://www.youtube.com/watch?v=abcdefghijk' },
          ],
        };
      },
    },
