...
export function BookPicker({ visible, bookList, books, onCancel, onSave }: BookPickerProps) {
  ...
  return (
    <AppFormModal
      ...
      onCancel={onCancel}
    >
      <div className="book-picker min-w-0 [gap:16px]">
        <Input
        ...
        />
        <div
          className="book-picker__list [min-height:180px] [max-height:min(420px,_55vh)] [gap:4px] mobile:[max-height:50dvh]"
          role="group"
          aria-label="选择书籍"
        >
          {filteredBooks.length ? (
            filteredBooks.map((book) => (
              <label
                className="book-picker__item min-w-0 [min-height:72px] [padding:4px_8px] mobile:[min-height:76px]"
                key={book.id}
              >
                <Checkbox
                ...
                <span className="book-picker__copy min-w-0 [gap:2px]">
        ...
        <div className="book-list-form__actions justify-end">
          <Text size="small" type="tertiary">
            已选择 {selectedIds.size} 本
          </Text>
          <span className="book-picker__buttons [margin-left:auto]">
