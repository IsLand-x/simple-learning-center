...
export function ImportBooksButton() {
  ...
  const finishBatch = (batch: ImportBatch) => {
    ...
    if (batch.failed.length) {
      ...
      Toast.error(
        `${batch.failed.length} 个文件无法导入：${preview}${batch.failed.length > 2 ? ' 等' : ''}`,
      );
    }
    setUploading(false);
    resetDialog();
  ...
  return (
    <>
      ...
      >
        <div className="epub-import-modal__body min-w-0 [gap:16px]">
          <Text type="tertiary">
            选择一本或多本 EPUB。解析出的书名、作者、目录和封面会随原文件一起保存。
          </Text>
          <Upload
            ref={uploadRef}
            ...
            disabled={uploading}
            fileListTitle={
              selectedFiles.length ? `已选择 ${selectedFiles.length} 本` : '待导入文件'
            }
            multiple
            ...
            onChange={({ fileList }) => {
              setSelectedFiles(
                fileList.flatMap((file) => (file.fileInstance ? [file.fileInstance] : [])),
              );
            }}
          >
            <Button
          ...
          <div
            className="epub-import-modal__progress min-w-0 [min-height:40px] justify-center [gap:6px]"
            aria-live="polite"
          >
            {progress ? (
              <>
                <div className="epub-import-modal__progress-label min-w-0 justify-between">
          ...
          <div className="epub-import-modal__actions min-w-0 justify-end">
            <Button disabled={uploading} theme="borderless" type="tertiary" onClick={resetDialog}>
              取消
            </Button>
            <Button
