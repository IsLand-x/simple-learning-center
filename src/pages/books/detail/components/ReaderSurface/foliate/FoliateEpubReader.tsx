...
import { useFoliateAnnotationSync } from './useFoliateAnnotationSync';
import { useFoliatePresentationSync } from './useFoliatePresentationSync';
import { useFoliateReaderController } from './useFoliateReaderController';
import { useFoliateReaderLifecycle, type FoliateReaderStatus } from './useFoliateReaderLifecycle';
...
export function FoliateEpubReader({
  ...
  return (
    <div
      ref={surfaceRef}
      className="foliate-reader-wrap relative overflow-hidden [background-color:var(--reader-paper-color,_var(--semi-color-bg-0))] [background-image:var(--reader-texture-image,_none)] [background-position:var(--reader-texture-position,_0_0)] [background-size:var(--reader-texture-size,_auto)] [background-blend-mode:var(--reader-texture-blend-mode,_normal)]"
      style={surfaceStyle}
      onPointerDown={onContentInteraction}
    >
      <span
        ref={commentIconTemplateRef}
        className="reader-comment-icon-template absolute [width:0] [height:0] overflow-hidden [visibility:hidden] pointer-events-none"
        aria-hidden="true"
      >
        <IconComment size="large" />
      </span>
      {status === 'loading' && (
        <div className="reader-status absolute [inset:0] [z-index:2] [place-items:center] [align-content:center] [background:var(--semi-color-bg-0)]">
          <Spin size="large" />
      ...
      <div
        ref={hostRef}
        className="foliate-reader-host relative [z-index:1] [background-color:transparent]"
      />
