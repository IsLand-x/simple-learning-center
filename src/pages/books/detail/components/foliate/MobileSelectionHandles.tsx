...
export function MobileSelectionHandles({
  ...
  const renderHandle = (endpoint: MobileSelectionHandleEndpoint) => {
    ...
    return (
      <button
        ...
        className={`mobile-selection-handle absolute [width:42px] [height:46px] [margin:0] [padding:0] [outline:0] [background:transparent] [color:var(--semi-color-primary)] [cursor:grab] [pointer-events:auto] [touch-action:none] [transform:translate(-50%,_-22px)] [-webkit-tap-highlight-color:transparent] mobile-selection-handle--${endpoint}`}
  ...
  return (
    <div
      className="mobile-selection-handles absolute [z-index:24] [inset:0] overflow-hidden pointer-events-none"
      aria-label="调整文本选区"
    >
      {renderHandle('start')}
      {renderHandle('end')}
