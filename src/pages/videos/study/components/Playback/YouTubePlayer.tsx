...
>(({ videoId, initialTime = 0, onTimeUpdate }, ref) => {
  ...
  const playerId = useMemo(
    () => `learning-center-youtube-${reactId.replace(/[^a-z0-9_-]/gi, '')}`,
    [reactId],
  );
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      if (
        ...
        !isYouTubeMessageOrigin(event.origin)
      )
        return;
  ...
  return (
    <div
      className="youtube-player relative w-full [min-height:220px] [flex:1_1_auto] overflow-hidden [background:var(--semi-color-bg-1)] mobile:min-h-0 mobile:[aspect-ratio:16_/_9]"
    ...
    >
      ...
      {status !== 'ready' && (
        <div
          className="youtube-player__status absolute [inset:0] justify-center [padding:20px] [background:var(--semi-color-bg-1)]"
          role={status === 'error' ? 'alert' : 'status'}
        >
          {status === 'loading' ? (
            <>
              <Spin />
              <Text type="tertiary">正在加载播放器…</Text>
            </>
          ) : (
            <Text type="danger">播放器加载失败，请检查网络后重试</Text>
          )}
        </div>
      )}
    </div>
