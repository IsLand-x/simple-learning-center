...
export function removeRssLeadingCover(value: string, coverUrl?: string) {
  ...
  const image = Array.from(
    document.body.querySelectorAll<HTMLImageElement>('img[data-rss-content-image="true"]'),
...
function highlightSearchMatches(document: Document, query: string) {
  ...
  while (node) {
    if (node instanceof Text && node.data && !node.parentElement?.closest('mark'))
      textNodes.push(node);
...
function resolvedAnnotationRange(value: string, annotation: RssContentAnnotation) {
  ...
  const matched =
    candidates.find(
      (index) =>
        (!annotation.prefix ||
          value.slice(Math.max(0, index - annotation.prefix.length), index) ===
            annotation.prefix) &&
        (!annotation.suffix ||
          value.slice(
...
function applyAnnotations(document: Document, annotations: RssContentAnnotation[]) {
  ...
  const ranges = annotations
    ...
    .filter((annotation): annotation is RssContentAnnotation & { start: number; end: number } =>
      Boolean(annotation),
    )
  ...
  textNodes.forEach(({ node: textNode, start: nodeStart, end: nodeEnd }) => {
    ...
    intersections.forEach((annotation) => {
      ...
      if (localStart > cursor)
        fragment.append(document.createTextNode(textNode.data.slice(cursor, localStart)));
      ...
      marker.setAttribute(
        'aria-label',
        annotation.kind === 'comment' ? '已评论内容' : '已高亮内容',
      );
...
export function sanitizeRssContentHtml(
  ...
  elements.forEach((element) => {
    ...
    if (tag === 'img') {
      const src = normalizeRssImageUrl(
        safeUrl(
  ...
  Array.from(document.body.querySelectorAll<HTMLElement>('h1, h2, h3')).forEach(
    (heading, index) => {
