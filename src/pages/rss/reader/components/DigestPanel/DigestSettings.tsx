...
import type { RssDigestRunStatus, RssDigestSettings } from '../../../../../../contracts/rss';
...
export function DigestSettings() {
  const workspace = useWorkspace().navigation;
  ...
  const visible = workspace.digestSettingsVisible;
  const onCancel: () => void = () => workspace.setDigestSettingsVisible(false);
  const onSave: (settings: RssDigestSettings) => void = (settings) => {
    setRssDigestSettings(settings);
    workspace.setDigestSettingsVisible(false);
  ...
  return (
    <SideSheet
      ...
      onCancel={onCancel}
    >
      <div className="rss-digest-settings min-h-0 [gap:20px] [padding:16px_20px_24px]">
        <section className="rss-digest-settings__section">
          <div className="rss-digest-settings__switch-row items-start">
            <div className="rss-digest-settings__switch-copy min-w-0">
              <Title heading={5}>自动生成今天的日报</Title>
              <Text size="small" type="tertiary">
                服务运行时会在后台整理当天未读内容，浏览器可以关闭。
              </Text>
            </div>
            <Switch
        ...
        <section className="rss-digest-settings__section">
          ...
          <Text size="small" type="tertiary">
            系统会额外要求模型读取全部条目、合并上一版日报、保留来源链接并对重复事件去重。
          </Text>
        </section>

        {draft.lastError && (
          <section className="rss-digest-settings__section rss-digest-settings__error [padding:12px] [background:var(--semi-color-danger-light-default)]">
        ...
        <section
          className="rss-digest-settings__section rss-digest-settings__history"
          aria-label="日报执行记录"
        >
          <div className="rss-digest-settings__history-heading justify-between">
            <Text strong>执行记录</Text>
            <Text size="small" type="tertiary">
              最近 {Math.min(runs.length, 100)} 条
            </Text>
          </div>
          {visibleRuns.length ? (
            <div className="rss-digest-runs" role="list">
              {visibleRuns.map((run) => {
                ...
                return (
                  <article
                    className="rss-digest-run min-w-0 [gap:4px] [padding:10px_0]"
                    key={run.id}
                    role="listitem"
                  >
                    <div className="rss-digest-run__topline justify-between">
                      <Tag color={meta.color} size="small" type="light">
                        {meta.label}
                      </Tag>
                      <time dateTime={new Date(run.startedAt).toISOString()}>
                        {runTimeLabel(run.startedAt)}
                      </time>
                    </div>
                    <Text strong>
                      {digestDateLabel(run.date)}日报 ·{' '}
                      {run.trigger === 'schedule' ? '定时任务' : '手动生成'}
                    </Text>
                    <Text size="small" type="tertiary">
                    ...
                    {run.message && (
                      <Text
                        className="rss-digest-run__message"
                        size="small"
                        type={run.status === 'failed' ? 'danger' : 'tertiary'}
                      >
                        {run.message}
                      </Text>
                    )}
            ...
            <Text size="small" type="tertiary">
              还没有日报执行记录。手动生成或定时任务运行后会显示在这里。
            </Text>
          )}
        </section>
      </div>
      <div className="rss-digest-settings__footer justify-end [padding:12px_20px_max(12px,_env(safe-area-inset-bottom))] [border-top:1px_solid_var(--semi-color-border)] [background:var(--semi-color-bg-1)]">
        ...
        <Button
          ...
          theme="solid"
          type="primary"
          onClick={() => onSave({ ...draft, prompt: draft.prompt.trim() })}
        >
          保存设置
        </Button>
      </div>
