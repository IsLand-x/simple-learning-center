...
import { HighlightedText } from '../ArticlePanel/HighlightedText';
import { digestDateLabel, digestPreview, feedTypeLabels, itemTime } from '../rssFormat';
import { getRssVideoPresentation } from '../rssVideo';
import { FeedTypeIcon } from '../SourcesPanel/FeedTypeIcon';
...
export function ItemList() {
  ...
  const dailyDigests = workspace.digestList;
  const feeds = persistedFeeds;
  const filteredItems = workspace.filteredItems;
  const query = workspace.query;
  const searchPreviews = workspace.searchPreviews;
  const selectedDigestId = workspace.selectedDigest?.id;
  const selectedFeedId = workspace.selectedFeedId;
  const selectedItemId = workspace.selectedItem?.id;
  const timeRange = workspace.timeRange;
  const todayItemsCount = workspace.todayItems.length;
  const todayKey = workspace.todayKey;
  const onOpenDigest = workspace.openDigest;
  const onOpenItem = workspace.openItem;
  const onOpenItemMenu: (item: RssItem, x: number, y: number) => void = (item, x, y) => {
    workspace.setSourceMenu(null);
    workspace.setItemMenu({ item, x, y });
  };
  const onSelectRange = workspace.selectRange;
