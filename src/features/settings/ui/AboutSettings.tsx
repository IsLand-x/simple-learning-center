...
import { requestSystemInfo, type SystemInfo } from '../application/systemInfo';
...
export function AboutSettings() {
  ...
  return (
    <section className="settings-about" aria-labelledby="settings-about-title">
      ...
      <section
        className="settings-about__machine"
              ...
              <dd>
                {info.addresses.length
                  ? info.addresses.map(({ name, address, family }) => (
                      <div className="settings-about__address" key={`${name}-${family}-${address}`}>
                        <code>{address}</code>
                        <Text size="small" type="tertiary">
                          {name} · {family}
                        </Text>
