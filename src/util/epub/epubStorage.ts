import { serverRequest } from '../api/serverApi';
...
export async function moveBookToTrash(bookId: string) {
  const response = await serverRequest(`/api/books/${encodeURIComponent(bookId)}/trash`, {
    method: 'POST',
  });
...
export async function restoreBookFromTrash(bookId: string) {
  const response = await serverRequest(`/api/books/${encodeURIComponent(bookId)}/restore`, {
    method: 'POST',
  });
  return response.json() as Promise<TrashedBookItem>;
}
...
export async function permanentlyDeleteBook(bookId: string) {
  const response = await serverRequest(`/api/books/${encodeURIComponent(bookId)}`, {
    method: 'DELETE',
  });
