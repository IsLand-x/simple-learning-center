...
function addFoliateAdjacentSectionPreview(code: string) {
  ...
  const fields = [
    ...
    '    #touchPagingLocked = false',
    '    #touchPagingLockedPosition = null',
    '    #touchPagingGuardFrame = null',
    "    #touchPagingIntent = 'pending'",
    '    #touchHoldTimer = null',
  ...
  const touchPagingReplacement = [
    ...
    '    }',
    '    #stopTouchPagingGuard() {',
    '        if (this.#touchPagingGuardFrame != null)',
    '            cancelAnimationFrame(this.#touchPagingGuardFrame)',
    '        this.#touchPagingGuardFrame = null',
    '    }',
    '    #startTouchPagingGuard() {',
    '        if (this.scrolled || this.#touchPagingGuardFrame != null) return',
    '        let remainingFrames = 3',
    '        const keepPosition = () => {',
    '            this.#touchPagingGuardFrame = null',
    '            const position = this.#touchPagingLockedPosition',
    '                ?? (this.#touchPagingBlocked ? this.#touchStartPosition : null)',
    '            if (position == null) return',
    '            if (this.#container[this.scrollProp] !== position)',
    '                this.#container[this.scrollProp] = position',
    '            remainingFrames -= 1',
    '            if (remainingFrames > 0)',
    '                this.#touchPagingGuardFrame = requestAnimationFrame(keepPosition)',
    '        }',
    '        this.#touchPagingGuardFrame = requestAnimationFrame(keepPosition)',
    '    }',
    '    cancelTouchPaging() {',
    ...
    "        this.#touchPagingIntent = 'selection'",
    '        this.cancelTouchScroll()',
    '        this.#touchScrolled = false',
    '        this.#startTouchPagingGuard()',
    '        if (this.#touchStartPosition != null)',
    '            this.containerPosition = this.#touchStartPosition',
    '    }',
    '    setTouchPagingBlocked(blocked) {',
    '        if (blocked && !this.#touchPagingLocked)',
    '            this.#touchPagingLockedPosition =',
    '                this.#touchStartPosition ?? this.containerPosition',
    '        this.#touchPagingLocked = blocked',
    '        if (blocked) this.cancelTouchPaging()',
    '        else {',
    '            this.#stopTouchPagingGuard()',
    '            this.#touchPagingLockedPosition = null',
    '            this.#touchPagingBlocked = false',
    "            this.#touchPagingIntent = 'pending'",
    '            this.#clearTouchHoldTimer()',
    ...
    '    #onTouchStart(e) {',
    '        this.cancelTouchScroll()',
    '        if (!this.#touchPagingLocked) {',
    '            this.#stopTouchPagingGuard()',
    '            this.#touchPagingLockedPosition = null',
    '        }',
    '        const doc = this.#view?.document',
    '        const selection = doc?.getSelection()',
    '        this.#touchPagingBlocked = this.#touchPagingLocked || Boolean(',
    ...
    "        this.#touchPagingIntent = this.#touchPagingBlocked ? 'selection' : 'pending'",
    '        this.#touchState = {',
    '            x: touch?.screenX, y: touch?.screenY,',
    '            t: e.timeStamp,',
    '            vx: 0, xy: 0,',
    ...
    "                if (this.#touchPagingIntent === 'pending') this.cancelTouchPaging()",
    '            }, 420)',
    '        }',
    '    }',
    '    #onTouchMove(e) {',
    ...
    '        if (!touch) return',
    '        const x = touch.screenX, y = touch.screenY',
    "        if (this.#touchPagingIntent === 'pending') {",
    '            const elapsed = e.timeStamp - this.#touchStartTime',
    '            const totalX = Math.abs(x - this.#touchStartX)',
    '            const totalY = Math.abs(y - this.#touchStartY)',
    '            if (elapsed > 280) {',
    '                this.cancelTouchPaging()',
    '                return',
    '            }',
    '            if (totalX < 18 || totalX < totalY * 1.35) return',
    "            this.#touchPagingIntent = 'page-turn'",
    '            this.#clearTouchHoldTimer()',
    '        }',
    "        if (this.#touchPagingIntent !== 'page-turn') return",
    '        e.preventDefault()',
    '        // Defer the visual page movement until touchend. Native mobile',
    '        // selection is exposed late, so moving here makes a long press',
    '        // flash an adjacent column before selection can cancel paging.',
    '        const dx = this.#touchStartX - x',
    '        const dy = this.#touchStartY - y',
    '        const dt = Math.max(1, e.timeStamp - this.#touchStartTime)',
    '        state.x = x',
    '        state.y = y',
    '        state.t = e.timeStamp',
    '        state.vx = dx / dt',
    ...
    '        const state = this.#touchState',
    '        if (!state) return',
    "        if (this.#touchPagingBlocked || this.#touchPagingIntent !== 'page-turn'",
    '            || !this.#touchScrolled) {',
    '            this.cancelTouchPaging()',
    '            return',
    '        }',
    ...
    '                this.snap(Math.sign(state.vx), Math.sign(state.vy))',
    '        })',
    '    }',
  ].join('\n');
  transformed = replaceFoliateSource(
  ...
  const scrollDispatchSource =
    "        this.#container.addEventListener('scroll', () => this.dispatchEvent(new Event('scroll')))";
  const scrollDispatchReplacement = [
    "        this.#container.addEventListener('scroll', () => {",
    '            const lockedPosition = this.#touchPagingLockedPosition',
    '                ?? (this.#touchPagingBlocked ? this.#touchStartPosition : null)',
    '            if (lockedPosition != null) {',
    '                if (this.#container[this.scrollProp] !== lockedPosition)',
    '                    this.#container[this.scrollProp] = lockedPosition',
    '                return',
    '            }',
  ...
  transformed = replaceFoliateSource(
    transformed,
    scrollDispatchSource,
    scrollDispatchReplacement,
    'Foliate paginator changed: locked selection scroll guard must be reviewed.',
  );

  const containerPositionSource = [
    '    set containerPosition(newVal) {',
    '        this.#container[this.scrollProp] = newVal',
    '    }',
  ].join('\n');
  const containerPositionReplacement = [
    '    set containerPosition(newVal) {',
    '        const lockedPosition = this.#touchPagingLockedPosition',
    '            ?? (this.#touchPagingBlocked ? this.#touchStartPosition : null)',
    '        const position = lockedPosition ?? newVal',
    '        this.#container[this.scrollProp] = position',
    '    }',
  ].join('\n');
  transformed = replaceFoliateSource(
    transformed,
    containerPositionSource,
    containerPositionReplacement,
    'Foliate paginator changed: locked container position must be reviewed.',
  );

  const touchListenerSource = [
  ...
  const touchListenerReplacement = [
    "        this.addEventListener('touchend', this.#onTouchEnd.bind(this))",
    "        this.addEventListener('touchcancel', () => this.cancelTouchPaging())",
    "        this.addEventListener('load', ({ detail: { doc } }) => {",
    "            doc.addEventListener('touchstart', this.#onTouchStart.bind(this), opts)",
    "            doc.addEventListener('touchmove', this.#onTouchMove.bind(this), opts)",
    "            doc.addEventListener('touchend', this.#onTouchEnd.bind(this))",
    "            doc.addEventListener('touchcancel', () => this.cancelTouchPaging())",
  ].join('\n');
  transformed = replaceFoliateSource(
    transformed,
    touchListenerSource,
    touchListenerReplacement,
    'Foliate paginator changed: touch cancellation listeners must be reviewed.',
  );

  // Foliate normally turns the page when a pointer selection appears to cross
  // the visible column. A paragraph split across CSS columns can make its
  // visible-range boundary land inside the current page, so an ordinary
  // selection is misclassified as crossing into the previous column. Since
  // cross-page selection is intentionally unsupported, disable this path for
  // every pointer type and keep paging under the reader's explicit controls.
